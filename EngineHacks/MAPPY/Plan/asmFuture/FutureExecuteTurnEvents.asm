...
.type FutureExecuteTurnEvents, %function
		...
		bne		VisibleEvent
			...
			beq		RemoveEventFromQueue
				...
				@SVAL s0 2: Do not count event for Future command usability, but act like SVAL s0 1
				cmp		r0, #2
				ble		GoToCallMapEventEngine
				
					@SVAL s0 3: Do not display turn number popup due to this event, but count this as a visible event
					cmp		r0, #3
					bne		VisibleEvent
